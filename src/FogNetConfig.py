from tensorflow.keras.layers import add, Reshape, BatchNormalization, Dense, concatenate
...
def SpatialAttentionBlock(input):
    # Attention Block
    # Maxpooling
    maxpool = MaxPooling3D(pool_size=(3, 3, 1), strides=(1, 1, 1), padding="same")(
        input
    )
    # print("The size of spatial maxpooling: ", maxpool._keras_shape)

    # Avgpooling
    avgpool = AveragePooling3D(pool_size=(3, 3, 1), strides=(1, 1, 1), padding="same")(
        input
    )
...
def SSTDilationBlock(input_map):
    ...
    Conv1_1 = Conv3D(
        filters=16, kernel_size=(3, 3, 1), dilation_rate=(1, 1, 1), padding="same"
    )(input_map_norm)
    Conv1_1 = PReLU()(Conv1_1)

    Conv1_2 = Conv3D(
        filters=16, kernel_size=(3, 3, 1), dilation_rate=(3, 3, 1), padding="same"
    )(input_map_norm)
    Conv1_2 = PReLU()(Conv1_2)

    Conv1_3 = Conv3D(
        filters=16, kernel_size=(3, 3, 1), dilation_rate=(5, 5, 1), padding="same"
    )(input_map_norm)
...
def NAMDilationBlock(input_map):
    input_map_norm = BatchNormalization()(input_map)
    NConv1_1 = Conv3D(
        filters=16, kernel_size=(3, 3, 3), dilation_rate=(1, 1, 1), padding="same"
    )(input_map_norm)
    NConv1_1 = PReLU()(NConv1_1)
    NConv1_2 = Conv3D(
        filters=16, kernel_size=(3, 3, 3), dilation_rate=(3, 3, 3), padding="same"
    )(input_map_norm)
    NConv1_2 = PReLU()(NConv1_2)
    NConv1_3 = Conv3D(
        filters=16, kernel_size=(3, 3, 3), dilation_rate=(5, 5, 5), padding="same"
    )(input_map_norm)
